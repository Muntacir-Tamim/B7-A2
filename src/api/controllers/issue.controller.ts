...
export const getIssueById = async (req: Request, res: Response) => {
  ...
  if (!issue) {
    return sendResponse(
      res,
      { message: "Issue not found", success: false, errors: "Not found" },
      404,
    );
  }
...
export const updateIssue = async (req: Request, res: Response) => {
  ...
  const isOwner =
    (existing.reporter_id as unknown as number) ===
  ...
  if (!isMaintainer) {
    ...
    if (existing.status !== "open") {
      return sendResponse(
        res,
        {
          message: "Conflict - you can only update issues with open status",
          success: false,
        },
        409,
      );
